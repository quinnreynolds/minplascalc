...
class BaseSpecies:
    ...
    def partitionfunction_translational(self, T):
        return (
            (2 * numpy.pi * self.molarmass * constants.Boltzmann * T)
            / (constants.Avogadro * constants.Planck**2)
        ) ** 1.5
...
class Polyatomic(Species):
    ...
    def partitionfunction_internal(self, T, dE):
        kbt = constants.Boltzmann * T
        electronicpartition = self.g0
        vibrationalpartition = numpy.prod(
            [
                numpy.exp(-wi / (2 * kbt)) / (1 - numpy.exp(-wi / kbt))
                for wi in self.wi_e
            ]
        )
        if self.linear_yn:
            ...
            rotationalpartition = (
                numpy.sqrt(constants.pi) / self.sigma_s * numpy.sqrt(kbt**3 / ABC)
            )
        return electronicpartition * vibrationalpartition * rotationalpartition
    ...
    def internal_energy(self, T, dE):
        ...
        vibrationalenergy = numpy.sum(
            [wi / (2 * numpy.tanh(wi / (2 * kbt))) for wi in self.wi_e]
        )
        return (
            translationalenergy
            + electronicenergy
